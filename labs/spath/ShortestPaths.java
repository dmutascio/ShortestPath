...
public class ShortestPaths {
    ...
    public void run() {
    	...
    	while (pq.isEmpty() != true) {
	    	...
	    	for (Edge e: v.vertex.edgesFrom()) {
	        	Decreaser<VertexAndDist> tempVertDistHandle = handles.get(e.to);
	        	VertexAndDist tempVertDist = tempVertDistHandle.getValue();
	    		if (weights.get(e) + v.distance<tempVertDist.distance) {
	    			tempVertDistHandle.decrease(new VertexAndDist(tempVertDist.vertex, weights.get(e) + v.distance));
	    			parentEdges.put(tempVertDist.vertex, e);
	    		}
	    	}
    	}
    ...
    public LinkedList<Edge> returnPath(Vertex endVertex) {
    	...
    	ArrayList<Edge> pathReverse = new ArrayList<Edge>();
    	while (currVertex != startVertex) {
    		pathReverse.add(parentEdges.get(currVertex));
    		currVertex=parentEdges.get(currVertex).from;
    	}
    	for (int i=pathReverse.size()-1; i>-1; i--) {
